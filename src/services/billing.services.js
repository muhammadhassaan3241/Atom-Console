...
import {
  ...
  getMonthYear,
} from '../middlewares/formatMonth.middleware.js';
...
export default {
  getInvoices: async (callback) => {
    try {
      ...
      await axios.get(invoicePath, headers).then((billing) => {
        const filteredData = [];
  ...
  getVpnActiveUsers: async (queryStrings, resellerId, callback) => {
    ...
    try {
      ...
      await axios
        .get(
          `${
            process.env.ATOM_BASE_URL
          }/vap/v1/listUsers?iResellerId=${reseller_id}&iPage=${
            page || 1
          }&iLimit=${limit || 100000}`,
          active_user_vpn_headers
        )
        .then(({ data }) => {
  ...
  getVpnConnectedUsers: async (resellerId, callback) => {
    ...
    const formattedFirstDay = `${firstDayOfMonth.getFullYear()}-${(
      ...
      .toString()
      .padStart(2, '0')}`;
    const formattedCurrentDay = `${today.getFullYear()}-${(today.getMonth() + 1)
      .toString()
    ...
    try {
      ...
      await axios
        .get(
          `${process.env.ES_BASE_URL}/networklogs/getResellerConnectedUsersList?IResellerId=${reseller_id}&sFromDate=${formattedFirstDay}&sToDate=${formattedCurrentDay}`,
          connected_user_vpn_headers
        )
        .then(({ data }) => {
        ...
        billingAccounts.map(async (account) => {
          try {
            if (
              ...
              const chargifyHeaders = {
                headers: {
                  'Content-Type': 'application/json',
                  Authorization:
                    'Basic empwQ1JpTXdVSWxoTVhjOFFDWDVrVE1reUdhTzFYbWhTcHM4WHJtd1U6Kg==',
                },
              };
              if (paid !== undefined) {
                await axios
                  .get(
                    `${process.env.CHARGIFY_BASE_URL}/components/${paid}/price_points.json`,
                    chargifyHeaders
                  )
                  .then(({ data }) => {
              ...
              if (trial !== undefined) {
                await axios
                  .get(
                    `${process.env.CHARGIFY_BASE_URL}/components/${trial}/price_points.json`,
                    chargifyHeaders
                  )
                  .then(({ data }) => {
                    const body = data.price_points;
                ...
                await axios
                  .get(
                    `${process.env.CHARGIFY_BASE_URL}/components/${connected}/price_points.json`,
                    chargifyHeaders
                  )
                  .then(({ data }) => {
                    const body = data.price_points;
              ...
              const getMonthStartAndEndDates = getMonthStartEndDates(
                startDateMonth.replace(/-0(\d)/, '-$1'),
                endDateMonth.replace(/-0(\d)/, '-$1')
              );
              ...
              for (const months of getMonthStartAndEndDates) {
                ...
                await axios
                  .get(
                    `${process.env.ES_BASE_URL}/networklogs/getResellerConnectedUsers?IResellerId=${resellerId}&sFromDate=${startDate}&sToDate=${endDate}`,
                  ...
                  .then(({ data }) => {
                    userAccountsTotalCost.push({
                      ...
                      total_cost_of_paid_accounts:
                        data.body.paid * paidUserUnitPrice || 0,
                    ...
                    userAccountsTotalCost.push({
                      ...
                      total_cost_of_trial_accounts:
                        data.body.trial * trialUserUnitPrice || 0,
                      // total_cost_of_connected_accounts: data.connected * connectedUserUnitPrice || 0,
                    });
                  });
              }
              ...
              if (active !== undefined) {
                await axios
                  .get(
                    `${process.env.CHARGIFY_BASE_URL}/components/${active}/price_points.json`,
                    chargifyHeaders
                  )
                  .then(({ data }) => {
                    const body = data.price_points;
                  ...
                  .then(({ data }) => {
                    userAccountsTotalCost.push({
                      ...
                      total_cost_of_active_accounts:
                        data.body.connected * activeUserUnitPrice || 0,
                    });
                  });
              }
            }
            return userAccountsTotalCost.length > 0
              ? callback(
                  userAccountsTotalCost,
                  200,
                  '1',
                  'Cost Of Monthly Account Record Found Sucessfully'
                )
              : callback(
                  userAccountsTotalCost,
                  404,
                  '0',
                  'Cost Of Monthly Account Record Not Found'
                );
          } catch (error) {
