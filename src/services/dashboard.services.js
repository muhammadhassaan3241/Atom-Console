...
export default {
  getMonthlyConnectedUsers: async (resellerId, callback) => {
    try {
      ...
      const formattedCurrentDay = `${today.getFullYear()}-${(
        today.getMonth() + 1
      )
        .toString()
      ...
      await axios
        .get(
          `${process.env.ES_BASE_URL}/networklogs/getResellerConnectedUsers?IResellerId=${reseller_id}&sFromDate=${formattedFirstDay}&sToDate=${formattedCurrentDay}`
        )
        .then(({ data }) => {
      ...
      fs.readFile('getCurrentLoadwrtSources.json', (error, data) => {
        if (error) {
          ...
          return callback(
            ...
            200,
            '1',
            'User Source Country List Found Successfully'
          );
        }
      });
    } catch (error) {
      ...
      fs.readFile('getCurrentLoadwrtDestination.json', (error, data) => {
        if (error) {
          return callback(
            ...
            404,
            '0',
            'User Destination Country List Not Found'
          );
        } else {
          const jsonData = JSON.parse(data);
          return callback(
            ...
            200,
            '1',
            'User Destination Country List Found Successfully'
          );
        }
      });
    } catch (error) {
